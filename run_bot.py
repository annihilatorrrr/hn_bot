...
def main():
    ...
    while 1:
        ...
        if bot_run_timer.limit_reached():
            ...
            run_logger.logger.info(
                "  --- :: Step 1 :: No Sleeping required before for the run can begin "
            )
        ...
        run_logger.logger.info(
            "  --- :: Step 2 :: Grabbing the top/first 500 posts on hacker_news "
        )
        ...
        if len( process_hn_posts.posts_to_process_list ) > 0:
            ...
            run_logger.logger.info(
                "  --- :: Step 3 :: Attempting to find the archived urls for all the previous posts "
            )
        ...
        if len(get_archived_urls.list_of_urls_to_save_wbm) > 0:
            ...
            if found:
                message_2 = "  --- :: Step 5 - 2:: We were able to save some urls using wayback_save() "
        ...
        message_3 = "  --- :: Step 6 :: We reset all the lists, for the next run, currently the list of prosts to prcess is empty, and all urls that were saved are now removed from the urls_to_save_wmb list"
        ...
        if run_logger.day_change_check() and error_logger.day_change_check():
            message_4 = "  --- :: Step 7 :: We changed the run_logger & error logger text files, since the day has changed"
