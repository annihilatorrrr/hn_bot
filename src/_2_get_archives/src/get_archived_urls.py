    ...
    def find_wayback(self, url): ### maybe pass in connection object --> connection handels api timer inside it ??
        ...
        if get_response.status_code == 200:
            ...
            if 'archived_snapshots' in wb_response_obj:  
                if archived_snapshop_map := wb_response_obj['archived_snapshots']:
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def find_archive(self, url):
        ...
        headers = {
            "User-Agent": User_Agent_list["linux"][2],
            "host": urlparse('https://archive.ph/').netloc,
            'Connection': 'close',
        }
        ...
        get_response = requests.post('https://archive.ph/submit/', **act_post_kwargs)
